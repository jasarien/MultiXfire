...
+ (id)loginPacketWithUsername:(NSString *)name password:(NSString *)pass flags:(NSUInteger)flg
{
...
+ (id)chatAcknowledgementPacketWithSID:(NSData *)sid imIndex:(NSUInteger)imidx
{
	XfireMutablePacket *pkt = [self packet];
...
+ (id)chatInstantMessagePacketWithSID:(NSData *)sid imIndex:(NSUInteger)imidx message:(NSString *)msg
{
	XfireMutablePacket *pkt = [self packet];
	XfirePacketAttributeMap *peermsg = [XfirePacketAttributeMap map];
