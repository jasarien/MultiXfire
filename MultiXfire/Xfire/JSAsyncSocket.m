...
@interface JSAsyncSocket (Private)
...
/**
 * The JSAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface JSAsyncReadPacket : NSObject
{
  @public
...
@implementation JSAsyncReadPacket
...
/**
 * The JSAsyncWritePacket encompasses the instructions for any given write.
**/
@interface JSAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation JSAsyncWritePacket
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[JSAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[JSAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	if (theDelegate == NULL)
    {
		[NSException raise:JSAsyncSocketException
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	else
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketCFSocketError userInfo:info];
		}
		return NO;
	}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		// Note: We use [self class] to support subclassing JSAsyncSocket.
		JSAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		NSLog(@"JSAsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"JSAsyncSocket %p couldn't attach read stream to run-loop,", self);
		NSLog (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"JSAsyncSocket %p couldn't attach write stream to run-loop,", self);
		NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		NSLog (@"JSAsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
		NSLog (@"JSAsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the JSAsyncSocket instance to be ready.
**/
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
			NSLog (@"JSAsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		NSLog(@"JSAsyncSocket couldn't determine IP version of socket");
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[JSAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"JSAsyncSocketCFSocketError",
														 @"JSAsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketCFSocketError userInfo:info];
}
...
/**
 * Returns a standard JSAsyncSocket abort error.
**/
- (NSError *)getAbortError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"JSAsyncSocketCanceledError",
														 @"JSAsyncSocket", [NSBundle mainBundle],
														 @"Connection canceled", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketCanceledError userInfo:info];
}

/**
 * Returns a standard JSAsyncSocket connect timeout error.
**/
- (NSError *)getConnectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"JSAsyncSocketConnectTimeoutError",
														 @"JSAsyncSocket", [NSBundle mainBundle],
														 @"Attempt to connect to host timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketConnectTimeoutError userInfo:info];
}

/**
 * Returns a standard JSAsyncSocket maxed out error.
**/
- (NSError *)getReadMaxedOutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"JSAsyncSocketReadMaxedOutError",
														 @"JSAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketReadMaxedOutError userInfo:info];
}

/**
 * Returns a standard JSAsyncSocket read timeout error.
**/
- (NSError *)getReadTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"JSAsyncSocketReadTimeoutError",
														 @"JSAsyncSocket", [NSBundle mainBundle],
														 @"Read operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketReadTimeoutError userInfo:info];
}

/**
 * Returns a standard JSAsyncSocket write timeout error.
**/
- (NSError *)getWriteTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"JSAsyncSocketWriteTimeoutError",
														 @"JSAsyncSocket", [NSBundle mainBundle],
														 @"Write operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:JSAsyncSocketErrorDomain code:JSAsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSUInteger)localAddressAsInt
{
	NSData *addressData = [self localAddress];
	NSUInteger addrInt = 0;
	const struct sockaddr_in *addr;
	addr = (const struct sockaddr_in*) [addressData bytes];
...
- (NSString *)description
{
	...
	[ms appendString:[NSString stringWithFormat:@"<JSAsyncSocket %p", self]];
                        ...
                        tag:(long)tag
{
	...
	JSAsyncReadPacket *packet = [[JSAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	JSAsyncWritePacket *packet = [[JSAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog(@"JSAsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog(@"JSAsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog(@"JSAsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	JSAsyncSocket *theSocket = [[(JSAsyncSocket *)pInfo retain] autorelease];
