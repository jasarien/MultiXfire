...
extern NSString* MFStringFromIPAddress(NSUInteger addr);
...
{	
	...
	switch ([err code]) {
		case JSAsyncSocketCFSocketError:
			reason = kXfireServerHungUpReason;
			break;
		case JSAsyncSocketConnectTimeoutError:
			reason = kXfireServerConnectionTimedOutReason;
			break;
		case JSAsyncSocketReadTimeoutError:
			reason = kXfireReadTimeOutReason;
			break;
		case JSAsyncSocketWriteTimeoutError:
...
- (void)processClanMembersPacket:(XfirePacket *)pkt
{
	...
	for (NSUInteger i = 0; i < [userIDs count]; i++)
	{
		...
		XfireFriend *clanMember = [_session friendForUserID:[userID intValue]];
		if (!clanMember)
		{
			clanMember = [[[XfireFriend alloc] init] autorelease];
		}
		
		[clanMember setUserID:[userID intValue]];
		[clanMember setIsClanMember:YES];
		[clanMember addClanID:[clanID intValue]];
