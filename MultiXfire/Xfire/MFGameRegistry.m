...
- (NSImage *)iconForGameID:(NSUInteger)gid
{
	NSMutableDictionary *gameInfo = (NSMutableDictionary *)[self infoForGameID:gid];
	NSImage *icon = [gameInfo objectForKey:kMFGameRegistryIconKey];
	if (!icon)
	{
...
+ (NSDictionary *)infoForGameID:(NSUInteger)gid
{
...
- (NSDictionary *)infoForGameID:(NSUInteger)gid
{
	return [_games objectForKey:[NSNumber numberWithUnsignedInteger:gid]];
}

+ (NSString *)longNameForGameID:(NSUInteger)gid
{
