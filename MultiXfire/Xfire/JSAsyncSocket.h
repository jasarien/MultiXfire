...
@class JSAsyncSocket;
@class JSAsyncReadPacket;
@class JSAsyncWritePacket;

extern NSString *const JSAsyncSocketException;
extern NSString *const JSAsyncSocketErrorDomain;

enum JSAsyncSocketError
{
	JSAsyncSocketCFSocketError = kCFSocketError,	// From CFSocketError enum.
	JSAsyncSocketNoError = 0,						// Never used.
	JSAsyncSocketCanceledError,					// onSocketWillConnect: returned NO.
	JSAsyncSocketConnectTimeoutError,
	JSAsyncSocketReadMaxedOutError,               // Reached set maxLength without completing
	JSAsyncSocketReadTimeoutError,
	JSAsyncSocketWriteTimeoutError
};
typedef enum JSAsyncSocketError JSAsyncSocketError;

@interface NSObject (JSAsyncSocketDelegate)
...
**/
- (void)onSocket:(JSAsyncSocket *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (void)onSocketDidDisconnect:(JSAsyncSocket *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(JSAsyncSocket *)sock didAcceptNewSocket:(JSAsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(JSAsyncSocket *)sock wantsRunLoopForNewSocket:(JSAsyncSocket *)newSocket;

/**
 * Called when a socket is about to connect. This method should return YES to continue, or NO to abort.
 * If aborted, will result in JSAsyncSocketCanceledError.
 * 
...
**/
- (BOOL)onSocketWillConnect:(JSAsyncSocket *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(JSAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(JSAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(JSAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(JSAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(JSAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(JSAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(JSAsyncSocket *)sock;
...
@interface JSAsyncSocket : NSObject
{
	...
	JSAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	JSAsyncWritePacket *theCurrentWrite;
...
// Once one of the accept or connect methods are called, the JSAsyncSocket instance is locked in
...
// When an incoming connection is accepted, JSAsyncSocket invokes several delegate methods.
 ...
 * When a connection comes in, the JSAsyncSocket instance will call the various delegate methods (see above).
 ...
 * 
 * Please note the recommended way of releasing an JSAsyncSocket instance (e.g. in a dealloc method)
 * [JSAsyncSocket setDelegate:nil];
 * [JSAsyncSocket disconnect];
 * [JSAsyncSocket release];
**/
- (void)disconnect;
 ...
 * 
 * If you pass a buffer, you must not alter it in any way while JSAsyncSocket is using it.
 ...
 * it is treated similarly to a timeout - the socket is closed with a JSAsyncSocketReadMaxedOutError.
 ...
 * The X509Certificate class is part of the CocoaJSAsyncSocket open source project.
**/
- (void)startTLS:(NSDictionary *)tlsSettings;

/**
 * For handling readDataToData requests, data is necessarily read from the socket in small increments.
 * The performance can be much improved by allowing JSAsyncSocket to read larger chunks at a time and
...
/**
 * When you create an JSAsyncSocket, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the JSAsyncSocket instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
