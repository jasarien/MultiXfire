...
- (XfirePacketAttributeMap *)scanAttributes:(NSUInteger)count binaryDeyDomain:(BOOL)binaryKeyDomain;
...
- (NSUInteger)scanUInt32;
- (NSUInteger)scanUInt64;
- (NSData *)scanUUID; // well, a 16 byte/128 bit value, anyway
- (NSString *)scanAttrKeyString;
- (NSString *)scanString;

- (NSData *)scanDataOfLength:(NSUInteger)len;
...
- (NSUInteger)scanUInt32
{
	...
	NSUInteger v = (
		((NSUInteger)_bytes[_idx]) |
		(((NSUInteger)_bytes[_idx+1]) << 8) |
		(((NSUInteger)_bytes[_idx+2]) << 16) |
		(((NSUInteger)_bytes[_idx+3]) << 24)
		);
...
- (NSUInteger)scanUInt64
{
	...
	NSUInteger v = (
		...
		(((NSUInteger)_bytes[_idx+2]) << 16) |
		(((NSUInteger)_bytes[_idx+3]) << 24) |
		(((NSUInteger)_bytes[_idx+4]) << 32) |
		(((NSUInteger)_bytes[_idx+5]) << 40) |
		(((NSUInteger)_bytes[_idx+6]) << 48) |
		(((NSUInteger)_bytes[_idx+7]) << 56)
		);
...
- (NSString *)scanString
{
	NSUInteger len = [self scanUInt16];
	NSString *s;
...
- (BOOL)scan:(NSUInteger *)packetID attributes:(XfirePacketAttributeMap **)attrsOut
{
	unsigned short len;
	...
	NSUInteger pktID   = (NSUInteger)[self scanUInt16];
...
- (XfirePacketAttributeMap *)scanAttributeMapInDomain:(XfireAttributeKeyDomain)domain
{
	...
	attrCnt = (NSUInteger)[self scanUInt8];
...
- (void)scanAttribute:(NSString **)keyOut keyDomain:(XfireAttributeKeyDomain)domain value:(id *)valueOut
{
	...
	if( domain == kXfireAttributeKeyIntegerDomain )
	{
		NSUInteger keyInt = (NSUInteger)[self scanUInt8];
