...
typedef NSUInteger XfirePacketID;
...
+ (id)loginPacketWithUsername:(NSString *)name password:(NSString *)pass flags:(NSUInteger)f;
	// name is username
...
+ (id)chatAcknowledgementPacketWithSID:(NSData *)sid imIndex:(NSUInteger)idx;
	// send an instant message
+ (id)chatInstantMessagePacketWithSID:(NSData *)sid imIndex:(NSUInteger)idx message:(NSString *)msg;
	// send a typing notification
+ (id)chatTypingNotificationPacketWithSID:(NSData *)sid imIndex:(NSUInteger)imidx typing:(NSUInteger)typing;

// client version packet (ID 3)
+ (id)clientVersionPacket:(NSUInteger)vers;
	// Not sure what a valid version value is; 82 came from a recent Xfire client

// game status change packet (ID 4)
+ (id)gameStatusChangePacketWithGameID:(NSInteger)gid gameIP:(NSInteger)gip gamePort:(NSInteger)port;
...
+ (id)removeFriendRequestWithUserID:(NSUInteger)uid;
...
+ (id)keepAlivePacketWithValue:(NSInteger)val stats:(NSArray *)stats;
	// pass 0 for val
...
+ (id)networkInfoPacketWithConn:(NSInteger)conn nat:(BOOL)isNat sec:(NSInteger)sec ip:(NSInteger)ip naterr:(BOOL)nErr uPnPInfo:(NSString *)info;
...
+ (id)removeCustomFriendGroupPacket:(NSUInteger)groupID;

// rename custom friend group packet (ID 28)
+ (id)renameCustomFriendGroupPacket:(NSUInteger)groupID newName:(NSString *)groupName;

// add friend to custom friend group (ID 29)
+ (id)addFriendPacket:(NSUInteger)friendID toCustomGroup:(NSUInteger)groupID;

// remove friend from custom friend group (ID 30)
+ (id)removeFriendPacket:(NSUInteger)friendID fromCustomGroup:(NSUInteger)groupID;
...
+ (id)infoViewRequestPacket:(NSUInteger)friendID;
