...
- (NSData *)compressedZlibData
{
	...
	compressionState.avail_in = (unsigned int)[self length];
	
	//outputBufferSize = [self length]*2;
	outputBufferSize = 12+(NSInteger)(1.2*(double)[self length]);
	//outputBufferSize = (1024*1024);
	outputBuffer = malloc(outputBufferSize);
	while(1)
	{
		compressionState.next_out = outputBuffer;
		compressionState.avail_out = (unsigned int)outputBufferSize;
...
+ (NSData *)archivedDataWithFiles:(NSArray *)paths
{
	...
	[emitter emitUInt32:(UInt32)[paths count]];
