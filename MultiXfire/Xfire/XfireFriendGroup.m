...
- (id)initCustomWithID:(NSUInteger)groupID name:(NSString *)groupName session:(XfireSession *)session
{
	self = [super init];
	if( self )
	{
		...
		_friendSortSelector = @selector(compareFriendsByDisplayName:);
		_session = session;
	}
	return self;
}
...
- (id)initWithClanID:(NSUInteger)clanID name:(NSString *)name shortName:(NSString *)shortName session:(XfireSession *)session
{
	if (self = [super init])
	{
		_groupID = clanID;
...
- (XfireFriend *)memberAtIndex:(NSInteger)idx
{
...
- (void)addPendingMemberID:(NSUInteger)userID
{
	[_pendingMembers addObject:[NSNumber numberWithUnsignedInteger:userID]];
}

- (NSUInteger)indexOfPendingMember:(NSUInteger)userID
{
	NSUInteger i, cnt;
	cnt = [_pendingMembers count];
	for( i = 0; i < cnt; i++ )
	{
		if( [[_pendingMembers objectAtIndex:i] unsignedIntegerValue] == userID )
		{
			return i;
		}
	}
...
- (BOOL)friendShouldBeMember:(XfireFriend *)fr
{
	...
	{
		for (NSNumber *clanID in [fr clanIDs])
		{
			if ([fr isClanMember] && ([clanID intValue] == [self groupID]))
			{
				return YES;
			}
		}
